...
const createSchema = z.object({
  username: z.string().min(3).max(50).regex(/^[a-zA-Z0-9_]+$/, 'Username may only contain letters, numbers, underscores'),
...
export async function POST(request: NextRequest) {
  ...
  await logAudit({
    userId, action: 'CREATE', entityType: 'user', entityId: user.id,
